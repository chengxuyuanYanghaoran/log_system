...
window.onload=function () {
    $.ajax({
        ...
        success: function (res) {
            ...
            totalData = res.pp;
            $("tbody").html("");
            $.each(res.monthlyPlanExtends, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    ...
                    "<td name='column'>" + obj["monthly_plan_time"] + "</td>" +
                    "<td name='column'>" + obj["monthly_plan_entry_time"] + "</td>" +
                    ...
                    "<td id='"+obj["monthly_plan_code"]+"abc"+"\'>默认</td>"+
                ...
                //判断日志状态
                if(obj["monthly_plan_state"]==1){
                    document.getElementById(obj["monthly_plan_code"]+"abc").innerHTML= "待提交";
                }else if (obj["monthly_plan_state"]==2){
                    document.getElementById(obj["monthly_plan_code"]+"abc").innerHTML = "待审核";
                }else if (obj["monthly_plan_state"]==3){
                    document.getElementById(obj["monthly_plan_code"]+"abc").innerHTML = "已驳回";
            ...
            layui.use('layer', function() { //独立版的layer无需执行这一句
                ...
                $("a[class='month_a']").on('click', function(){
                    ...
                    $.ajax({
                        async: false,
                        url: "/system/monthly/getMonthlyPlanById",
                        data:jsonData,
                        type:"post",
                        datatype:"json",
                        ...
                        success: function (res) {
                            if (res.code==null){
                                ...
                                document.getElementById("oneP_plan").innerHTML= res.monthlyPlan1.monthly_plan_content;
                                document.getElementById("oneP_end").innerHTML = res.monthlyPlan1.monthly_plan_entry_time;
                            }
                        }
                    });
...
//    ajax
    $.ajax({
        url:"/system/monthly/compositeQuery",
        type:"post",
        datatype:"json",
        ...
        success:function (res) {
            ...
            $.each(res.monthlyPlan1, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    "<td name='operate_check_column'><input name='operate_check'type='checkbox' value='"+obj["id"]+"'></td>"+
                    "<td name='column' id='"+obj["id"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
...
function Get_cancel(res) { //审核通过
    var Get_cancel =4;
    var id =Number(res);
    var category={"id":id,"monthly_plan_state":Get_cancel};
    var jsonData = JSON.stringify(category);

    $.ajax({
        url:"/system/monthly/updateMonthlyPlanStateById",
        type:"post",
        data:jsonData,
        datatype:"json",
        contentType : "application/json;charset=UTF-8",
...
function handleData(data1,data2) {
    $.ajax({
        ...
        success:function (data) {
            ...
            totalData = data.pp;
            $("tbody").html("");
            $.each(data.monthlyPlanExtends, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    "<td name='operate_check_column'><input name='operate_check'type='checkbox' value='"+obj["id"]+"'></td>"+
                    "<td name='column' id='"+obj["id"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
