...
window.onload=function () {
    $.ajax({
        async : false,
        url: "/system/monthlySummary/list",
        type: "POST",
        data:{
            page:page,
            pageSize:pageSize
        ...
        success: function (res) {
            ...
            $.each(res.monthlySummaryExtendDOS, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    ...
                    "<td name='column'>" + obj["monthly_summary_time"] + "</td>" +
                    "<td name='column'>" + obj["monthly_summary_entry_time"] + "</td>" +
                    ...
                    "<td id='"+obj["monthly_summary_code"]+"abc"+"\'>默认</td>"+
                ...
                //判断日志状态
                if(obj["monthly_summary_state"]==1){
            ...
            layui.use('layer', function() { //独立版的layer无需执行这一句
                ...
                $("a[class='month_a']").on('click', function(){
                    ...
                    $.ajax({
                        async: false,
                        url: "/system/monthlySummary/getMonthlySummaryById",
                        data:jsonData,
                        type:"post",
                        datatype:"json",
...
//    ajax
    $.ajax({
        url:"/system/monthlySummary/compositeQueryMonthlySummary",
        type:"post",
        datatype:"json",
        ...
        success:function (res) {
            ...
            $.each(res.monthlySummaryExtendDOS, function (index, obj) {
                ...
                if(obj["monthly_summary_state"]==1){
                    ...
                    document.getElementById(obj["monthly_summary_code"]+"abc").innerHTML = "错误";
                }
...
function Get_cancel(res) { //审核通过
    var Get_cancel =4;
    var id =Number(res);
    var category={"id":id,"monthly_summary_state":Get_cancel};
    var jsonData = JSON.stringify(category);

    $.ajax({
        url:"/system/monthlySummary/updateMonthlySummaryStateById",
        type:"post",
        data:jsonData,
        datatype:"json",
        contentType : "application/json;charset=UTF-8",
...
function handleData(data1,data2) {
    $.ajax({
        ...
        success:function (data) {
            ...
            totalData = data.pp;
            $("tbody").html("");
            $.each(data.monthlySummaryExtendDOS, function (index, obj) {
                $("#table_id").append("<tr onmodify='false' id='trOne'>"+
                    "<td name='operate_check_column'><input name='operate_check'type='checkbox' value='"+obj["id"]+"'></td>"+
                    "<td name='column' id='"+obj["id"]+"\"'>" + obj["jobNumber"] + "</td>" +
                    "<td name='column'>" + obj["peoName"] + "</td>" +
    ...
    var active = {
        setTop: function () {
            ...
            layer.open({
                type: 1 //此处以iframe举例
                , content:$("#layui_two")
                , title:  ['请添加月总结！', 'font-size:18px;color:green']
                , area: ['590px', '68%']
                , shade: 0
                , tipsMore: true
                , maxmin: true
...
function reportForm(id) {
    location.href = '/system/reportForm/sum_month01.html?param='+id;
}
