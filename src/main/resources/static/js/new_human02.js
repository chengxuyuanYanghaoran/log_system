        ...
        success: function (res) {
            ...
            $.each(res.weekPlanExtendDOS, function (index, obj) {
                ...
                if(obj["week_plan_state"]==1){
                    ...
                    document.getElementById(obj["week_plan_code"]+"abc").innerHTML = "已驳回";
                }else if (obj["week_plan_state"]==4){
                    document.getElementById(obj["week_plan_code"]+"abc").innerHTML = "已审核";
                }

                //判断颜色
                if (obj["week_plan_state"]==3 ){  //待审核状态
                    document.getElementById(obj["week_plan_code"]+"abc").style.color = "red";
                }else if (obj["week_plan_state"]==2){
                    document.getElementById(obj["week_plan_code"]+"abc").style.color = "blue";
                }else if (obj["week_plan_state"]==4){
                    document.getElementById(obj["week_plan_code"]+"abc").style.color = "green";
                }else if (obj["week_plan_state"]==1){
                    document.getElementById(obj["week_plan_code"]+"abc").style.color = "orange";
                }
            ...
            layui.use('layer', function() { //独立版的layer无需执行这一句
                ...
                $("a[class='month_a']").on('click', function(){
                    ...
                    $.ajax({
                        async: false,
                        url: "/system/weekPlan/getWeekPlanById",
                        data:jsonData,
                        type:"post",
                        datatype:"json",
                        ...
                        success: function (res) {
                            if (res.code==null){
                                ...
                                if (res.weekPlanExtendDO==null){
                                    alert(res.msg)
                                }else{
                                    document.getElementById("oneP_plan_week").innerHTML= res.weekPlanExtendDO.week_plan_content;
                                    document.getElementById("oneP_end_week").innerHTML=  res.weekPlanExtendDO.week_plan_entry_time;
                                    document.getElementById("oneP_plan").innerHTML= res.weekPlanExtendDO.monthlyPlan.monthly_plan_content;
                                    document.getElementById("oneP_end").innerHTML=res.weekPlanExtendDO.monthlyPlan.monthly_plan_entry_time;
                                }
...
//    ajax
    $.ajax({
        url:"/system/weekPlan/compositeQuery",
        type:"post",
        datatype:"json",
        ...
        success:function (res) {
            ...
            $.each(res.weekPlanExtendDOS, function (index, obj) {
                ...
                if(obj["week_plan_state"]==1){
                    ...
                    document.getElementById(obj["week_plan_code"]+"abc").innerHTML = "错误";
                }
                //判断颜色
                if (obj["week_plan_state"]==3 ){  //待审核状态
                    document.getElementById(obj["week_plan_code"]+"abc").style.color = "red";
...
function Get_cancel(res) { //审核通过
    var Get_cancel =4;
    var id =Number(res);
    var category={"id":id,"week_plan_state":Get_cancel};
    var jsonData = JSON.stringify(category);

    $.ajax({
        url:"/system/weekPlan/updateWeekPlanStateById",
        type:"post",
        data:jsonData,
        datatype:"json",
        contentType : "application/json;charset=UTF-8",
    ...
    var active = {
        setTop: function () {
            ...
            layer.open({
                type: 1 //此处以iframe举例
                , content:$("#layui")
                , title:  ['请添加周计划！', 'font-size:18px;color:red']
                , area: ['590px', '78%']
                , shade: 0
                , tipsMore: true
                , maxmin: true
...
function reportForm(id) {
    location.href = '/system/reportForm/sum_month02.html?param='+id;
}